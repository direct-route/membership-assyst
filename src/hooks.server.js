...
export async function handle({ event, resolve }) {
	...
	// Set refreshed cookie before resolve so headers aren't sealed
	if (authValid && pb.authStore.token && path !== '/signout') {
		event.cookies.set('pb_auth', JSON.stringify({ token: pb.authStore.token, record: pb.authStore.record }), COOKIE_OPTS);
	}
